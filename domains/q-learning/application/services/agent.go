...
type Rewarder interface {
	...
	// state.
	Reward(*StateAction, int64) float64
}
...
type Agent interface {
	...
	Learn(*StateAction, Rewarder, int64)
...
type SimpleAgent struct {
	q  map[string]map[int64]float64
	lr float64
	d  float64
}
...
func NewSimpleAgent(lr, d float64) *SimpleAgent {
	return &SimpleAgent{
		q:  make(map[string]map[int64]float64),
		d:  d,
		lr: lr,
	}
}
...
func (agent *SimpleAgent) getActions(state string) map[int64]float64 {
	if _, ok := agent.q[state]; !ok {
		agent.q[state] = make(map[int64]float64)
	}
...
func (agent *SimpleAgent) Learn(action *StateAction, reward Rewarder, signal int64) {
	...
	currentVal := actions[action.Action.Int()]
	actions[action.Action.Int()] = currentVal + agent.lr*(reward.Reward(action, signal)+agent.d*maxNextVal-currentVal)
}

// Value gets the current Q-value for a State and Action.
func (agent *SimpleAgent) Value(state State, action Action) float64 {
	return agent.getActions(state.String())[action.Int()]
}
